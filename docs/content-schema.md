...
| `publishedAt` | date | yes | Publication date, e.g. `2026-09-24`. Drives the sort order (newest first). |
| `updatedAt` | date \| null | no | Set when an article is meaningfully revised; otherwise `null`. |
...
Articles with `draft: true` never reach public output. They
...
The filtering happens in `src/utils/articles.ts`; no page is generated for a
draft, so exclusion holds for every consumer of the built site. Drafts are
therefore safe to keep on a branch, but they must never be pushed to a
public branch (see `docs/cloudflare-pages-setup.md`).
